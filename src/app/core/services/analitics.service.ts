...
export class AnalyticsService {
  calcularResumen(transacciones: Transaccion[]): ResumenFinanciero {
    ...
    const gastosPorCategoria = CATEGORIAS.map(cat => {
      const monto = transacciones.filter(t => t.tipo === 'gasto' && t.categoria === cat.key).reduce((sum, t) => sum + t.monto, 0);
      const porcentaje = totalGastos ? (monto / totalGastos) * 100 : 0;
      return { categoria: cat.key, monto, porcentaje, color: cat.color };
    });

    return { saldoActual, totalIngresos, totalGastos, gastosPorCategoria };
  }
}
